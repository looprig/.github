...
Exported named types with an explicit `Error() string` method are `AuthError`, `ConfigurationError`, `RequestError`. Use `errors.Is` or `errors.As` only when the relevant function or method returns one of these errors or wraps it.
