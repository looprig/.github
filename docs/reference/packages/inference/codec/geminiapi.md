...
Import path: `github.com/looprig/inference/codec/geminiapi`. This public package defines one part of the provider-neutral inference API.
...
`DecodeEvent`, `DecodeRequest`, `DecodeResponse`, `DecodeStream`, `EncodeRequest`, `Error`, `MatchRequest`, `OpenStream`, `Unwrap`, `WriteError`, `WriteResponse`

### Types {#types}

`Codec`, `DecodeError`, `DuplicateKeyError`, `EncodeError`, `GenerateContentRequest`, `GenerateContentResponse`, `ServerDecodeError`, `StreamTerminatedError`, `UnsupportedBlockError`, `UnsupportedChunkError`
