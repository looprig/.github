...
`IsError` is preserved. Required tool choice becomes `tool_choice: {type:"any"}`, and a named tool
choice becomes `tool_choice: {type:"tool", name:"..."}`.
...
is true and the request carries no non-empty transient system message, because
such a message folds into the top-level `system` prefix and caching either
breakpoint would capture transient context. It then emits at most two `cache_control: {type:"ephemeral"}`
breakpoints: one on the system block, and one on the last cacheable block of
the committed message history, so multi-turn requests accrue incremental cache
hits. Transient messages are never marked, and thinking and redacted-thinking
blocks are skipped. With the capability off, system remains a plain string and
...
- [`anthropicapi/types.go`](https://github.com/looprig/inference/blob/v0.14.0/codec/anthropicapi/types.go)
...
- [`anthropicapi/decode.go`](https://github.com/looprig/inference/blob/v0.14.0/codec/anthropicapi/decode.go)
