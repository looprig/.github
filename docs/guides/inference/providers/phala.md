---
id: guides/inference/providers/phala
title: Phala Confidential Inference
description: Configure the Phala Confidential Inference provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 45
...
# Phala Confidential Inference

Composes the ACI attested client with a caller-owned acceptance policy.

## Contract and endpoint

The public constructor is `func New(baseURL string, key auth.APIKey, p aci.Policy) (inference.Client, error)`. It binds provider identity `phala` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/phala` (package `phala`) |
| Provider | `phala` |
| Formats | OpenAI |
| Default base | https://inference.phala.com |
| Authentication | API key plus caller-supplied ACI policy |
| Header or signer | Bearer credential passed to ACI |
| Options | No public options; New(baseURL, key, aci.Policy) |

Empty key fails before base defaulting. Empty or unpinned policy returns ACI's typed policy error unless caller opts out; package ships no trust anchors.
...
No response cache is implemented. ACI policy and attestation state are separate from prompt caching.

## Counters, errors, and retries

No NewCounter is exported. ACI transport attestation is not token counting.
...
import (
    ...
    phala "github.com/looprig/llm/providers/phala"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("phala"), model.APIFormatOpenAI, "", "model-name")
    _ = selected
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := phala.New("", auth.APIKey(os.Getenv("PHALA_API_KEY")), aci.UnpinnedPolicy())
