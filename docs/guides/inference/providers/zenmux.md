---
id: guides/inference/providers/zenmux
title: ZenMux
description: Configure the ZenMux provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 56
...
Protocol-conversion wrapper choosing ZenMux OpenAI or Anthropic root and path from APIFormat.

## Contract and endpoint

The public constructor is `func New(selected model.Model, key auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `zenmux` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/zenmux` (package `zenmux`) |
| Provider | `zenmux` |
| Formats | OpenAI, OpenAI Responses, Anthropic |
| Default base | OpenAI https://zenmux.ai/api/v1; Anthropic https://zenmux.ai/api/anthropic/v1 |
| Authentication | API key |
| Header or signer | Authorization: Bearer |
| Options | WithHeader; WithReasoningEffort; WithThinkingBudget |

Anthropic root is separate and receives anthropic-version 2023-06-01.
...
import (
    ...
    zenmux "github.com/looprig/llm/providers/zenmux"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("zenmux"), model.APIFormatAnthropic, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := zenmux.New(selected, auth.APIKey(os.Getenv("ZENMUX_API_KEY")), zenmux.WithThinkingBudget(1024))
...
- [client.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/zenmux/client.go)
- [counter.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/zenmux/counter.go)
- [errors.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/zenmux/errors.go)
...
- [client_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/zenmux/client_test.go)
