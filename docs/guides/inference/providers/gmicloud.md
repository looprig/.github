---
id: guides/inference/providers/gmicloud
title: GMI Cloud
description: Configure the GMI Cloud provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 24
...
The public constructor is `func New(selected model.Model, key auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `gmicloud` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/gmicloud` (package `gmicloud`) |
| Provider | `gmicloud` |
| Formats | OpenAI |
| Default base | https://api.gmi-serving.com/v1 |
| Authentication | API key |
| Header or signer | Authorization: Bearer |
| Options | WithHeader; WithOrganizationID; WithTopK |

WithOrganizationID and WithTopK patch the OpenAI request body and do not alter model identity.
...
import (
    ...
    gmicloud "github.com/looprig/llm/providers/gmicloud"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("gmicloud"), model.APIFormatOpenAI, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := gmicloud.New(selected, auth.APIKey(os.Getenv("GMICLOUD_API_KEY")))
...
- [client.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/gmicloud/client.go)
- [counter.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/gmicloud/counter.go)
- [errors.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/gmicloud/errors.go)
- [options.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/gmicloud/options.go)
...
- [client_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/gmicloud/client_test.go)
- [counter_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/gmicloud/counter_test.go)
