---
id: guides/inference/providers/opencode-go
title: OpenCode Go
description: Configure the OpenCode Go provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 40
...
Protocol-conversion wrapper for OpenCode Go with three bundled formats.

## Contract and endpoint

The public constructor is `func New(selected model.Model, key auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `opencode-go` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/opencode-go` (package `opencode`) |
| Provider | `opencode-go` |
| Formats | OpenAI, OpenAI Responses, Anthropic |
| Default base | https://opencode.ai/zen/go/v1 |
| Authentication | API key |
| Header or signer | Authorization: Bearer |
| Options | WithHeader |
| Conversation header | `x-opencode-session` from `inference.Request.SessionID` |

Provider identity and endpoint are distinct from opencode; model validation prevents crossing them.
...
import (
    ...
    opencode "github.com/looprig/llm/providers/opencode-go"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("opencode-go"), model.APIFormatOpenAI, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := opencode.New(selected, auth.APIKey(os.Getenv("OPENCODE_GO_API_KEY")))
...
- [client.go](https://github.com/looprig/llm/blob/v0.15.0/providers/opencode-go/client.go)
- [opencodesession/session.go](https://github.com/looprig/llm/blob/v0.15.0/providers/internal/opencodesession/session.go)
- [counter.go](https://github.com/looprig/llm/blob/v0.15.0/providers/opencode-go/counter.go)
- [errors.go](https://github.com/looprig/llm/blob/v0.15.0/providers/opencode-go/errors.go)
- [options.go](https://github.com/looprig/llm/blob/v0.15.0/providers/opencode-go/options.go)
...
- [client_test.go](https://github.com/looprig/llm/blob/v0.15.0/providers/opencode-go/client_test.go)
- [counter_test.go](https://github.com/looprig/llm/blob/v0.15.0/providers/opencode-go/counter_test.go)
- [session_header_test.go](https://github.com/looprig/llm/blob/v0.15.0/providers/opencode-go/session_header_test.go) runs the shared [session-header contract](https://github.com/looprig/llm/blob/v0.15.0/providers/internal/contracttest/session.go)
