---
id: guides/inference/providers/google-vertex
title: Google Vertex AI
description: Configure the Google Vertex AI provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 25
...
Vertex client routes Gemini generateContent and Anthropic Claude rawPredict with project and location path segments.

## Contract and endpoint

The public constructor is `func New(selected model.Model, accessToken auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `google-vertex, google-vertex-anthropic` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/google-vertex` (package `vertex`) |
| Provider | `google-vertex, google-vertex-anthropic` |
| Formats | Gemini, Anthropic |
| Default base | dynamic: location-specific or global Vertex AI host |
| Authentication | GCP bearer |
| Header or signer | Authorization: Bearer OAuth access token |
| Options | WithProject; WithLocation; WithHeader; WithAnthropicVersion |

GOOGLE_OAUTH_ACCESS_TOKEN, GOOGLE_CLOUD_PROJECT, and GOOGLE_CLOUD_LOCATION are fallbacks. google-vertex-anthropic is restricted to Anthropic by model validation.
...
The llm provider registry classifies this identity as requiring GCP bearer. The constructor receives resolved credential material rather than a secret reference. A credentials or secrets integration can resolve a descriptor and lease before passing the value here; secret labels do not belong in model.Model.

| Decision | Result |
| --- | --- |
| Credential | GCP bearer |
| Wire auth | Authorization: Bearer OAuth access token |
| Format gate | Gemini, Anthropic; unsupported values fail model validation before I/O |
...
func invoke() error {
    ...
    selected := model.CustomModel(model.ProviderName("google-vertex"), model.APIFormatGemini, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := vertex.New(selected, auth.APIKey(os.Getenv("GOOGLE_OAUTH_ACCESS_TOKEN")), vertex.WithProject("project"), vertex.WithLocation("us-central1"))
...
- [client.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/google-vertex/client.go)
- [counter.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/google-vertex/counter.go)
- [errors.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/google-vertex/errors.go)
...
- [client_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/google-vertex/client_test.go)
- [validation_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/google-vertex/validation_test.go)
