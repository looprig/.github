---
id: guides/inference/providers/gemini
title: Google Gemini
description: Configure the Google Gemini provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 21
...
Bespoke Gemini generateContent client. It appends /models/{name}:generateContent or :streamGenerateContent?alt=sse and delegates body and event decoding to the Gemini codec.

## Contract and endpoint

The public constructor is `func New(key auth.APIKey) (inference.Client, error)`. It binds provider identity `google` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/gemini` (package `gemini`) |
| Provider | `google` |
| Formats | Gemini |
| Default base | https://generativelanguage.googleapis.com/v1beta |
| Authentication | API key header |
| Header or signer | x-goog-api-key |
| Options | WithHeader |

Provider identity google is distinct from Vertex. Gemini streaming ends at SSE body EOF because it has no [DONE] sentinel.
...
The llm provider registry classifies this identity as requiring API key header. The constructor receives resolved credential material rather than a secret reference. A credentials or secrets integration can resolve a descriptor and lease before passing the value here; secret labels do not belong in model.Model.

| Decision | Result |
| --- | --- |
| Credential | API key header |
| Wire auth | x-goog-api-key |
| Format gate | Gemini; unsupported values fail model validation before I/O |
...
NewCounter sends exact Gemini countTokens requests and returns typed state, endpoint, request, and response errors.
...
func invoke() error {
    ...
    selected := model.CustomModel(model.ProviderName("google"), model.APIFormatGemini, "", "model-name")
    _ = selected
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := gemini.New(auth.APIKey(os.Getenv("GOOGLE_API_KEY")))
