---
id: guides/inference/providers/vercel
title: Vercel AI Gateway
description: Configure the Vercel AI Gateway provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 53
...
Three-format gateway wrapper selecting Messages, Chat, or Responses paths.

## Contract and endpoint

The public constructor is `func New(selected model.Model, key auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `vercel` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/vercel` (package `vercel`) |
| Provider | `vercel` |
| Formats | OpenAI, OpenAI Responses, Anthropic |
| Default base | https://ai-gateway.vercel.sh/v1 |
| Authentication | API key |
| Header or signer | Authorization: Bearer |
| Options | WithHeader; WithReasoningEffort; WithMetadata |

DefaultOpenAIBaseURL and DefaultAnthropicBaseURL are explicit constants. No tokenizer or cache control is exported.
...
import (
    ...
    vercel "github.com/looprig/llm/providers/vercel"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("vercel"), model.APIFormatAnthropic, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := vercel.New(selected, auth.APIKey(os.Getenv("VERCEL_API_KEY")))
...
- [client.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/vercel/client.go)
- [counter.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/vercel/counter.go)
- [errors.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/vercel/errors.go)
...
- [client_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/vercel/client_test.go)
