---
id: guides/inference/providers/together
title: Together AI
description: Configure the Together AI provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 51
...
The public constructor is `func New(selected model.Model, key auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `togetherai` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/together` (package `together`) |
| Provider | `togetherai` |
| Formats | OpenAI |
| Default base | https://api.together.ai/v1 |
| Authentication | API key |
| Header or signer | Authorization: Bearer |
| Options | WithHeader; WithReasoningEffort; WithReasoningEnabled |

No Responses or Anthropic format is registered.
...
import (
    ...
    together "github.com/looprig/llm/providers/together"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("togetherai"), model.APIFormatOpenAI, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := together.New(selected, auth.APIKey(os.Getenv("TOGETHER_API_KEY")))
