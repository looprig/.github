---
id: guides/inference/providers/venice
title: Venice
description: Configure the Venice provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 52
...
Dual-format Venice client selecting Chat or Responses routing and patching provider parameters.

## Contract and endpoint

The public constructor is `func New(selected model.Model, key auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `venice` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/venice` (package `venice`) |
| Provider | `venice` |
| Formats | OpenAI, OpenAI Responses |
| Default base | https://api.venice.ai/api/v1 |
| Authentication | API key |
| Header or signer | Authorization: Bearer |
| Options | WithHeader; WithReasoningEffort; WithServiceTier; WithVeniceParameters |

No cache-specific option or Anthropic format is registered.
...
import (
    ...
    venice "github.com/looprig/llm/providers/venice"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("venice"), model.APIFormatOpenAIResponses, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := venice.New(selected, auth.APIKey(os.Getenv("VENICE_API_KEY")))
...
- [client.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/venice/client.go)
- [counter.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/venice/counter.go)
- [errors.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/venice/errors.go)
...
- [client_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/venice/client_test.go)
