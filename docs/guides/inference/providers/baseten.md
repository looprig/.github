---
id: guides/inference/providers/baseten
title: Baseten
description: Configure the Baseten provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 9
...
The public constructor is `func New(selected model.Model, key auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `baseten` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/baseten` (package `baseten`) |
| Provider | `baseten` |
| Formats | OpenAI |
| Default base | https://inference.baseten.co/v1 |
...
import (
    ...
    baseten "github.com/looprig/llm/providers/baseten"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("baseten"), model.APIFormatOpenAI, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := baseten.New(selected, auth.APIKey(os.Getenv("BASETEN_API_KEY")))
