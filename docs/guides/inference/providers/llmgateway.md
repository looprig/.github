---
id: guides/inference/providers/llmgateway
title: LLM Gateway
description: Configure the LLM Gateway provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 32
...
Dual-format gateway wrapper. OpenAI selects /chat/completions; Anthropic selects /messages and native version header.

## Contract and endpoint

The public constructor is `func New(selected model.Model, key auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `llmgateway` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/llmgateway` (package `llmgateway`) |
| Provider | `llmgateway` |
| Formats | OpenAI, Anthropic |
| Default base | https://api.llmgateway.io/v1 |
| Authentication | API key |
| Header or signer | Authorization: Bearer |
| Options | WithHeader; WithReasoningEffort; WithThinkingBudget |

OpenAI Responses and provider-specific cache controls are not registered.
...
import (
    ...
    llmgateway "github.com/looprig/llm/providers/llmgateway"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("llmgateway"), model.APIFormatOpenAI, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := llmgateway.New(selected, auth.APIKey(os.Getenv("LLMGATEWAY_API_KEY")))
...
- [client.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/llmgateway/client.go)
- [counter.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/llmgateway/counter.go)
- [errors.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/llmgateway/errors.go)
...
- [client_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/llmgateway/client_test.go)
