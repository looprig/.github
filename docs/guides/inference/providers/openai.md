---
id: guides/inference/providers/openai
title: OpenAI
description: Configure the OpenAI provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 39
...
Dual-codec OpenAI client. APIFormat chooses /chat/completions or /responses and options patch the matching body.

## Contract and endpoint

The public constructor is `func New(selected model.Model, key auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `openai` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/openai` (package `openai`) |
| Provider | `openai` |
| Formats | OpenAI, OpenAI Responses |
| Default base | https://api.openai.com/v1 |
| Authentication | API key |
| Header or signer | Authorization: Bearer |
| Options | WithRoundTripper; WithTLSRootCAs; WithReasoning; WithServiceTier; WithMetadata; WithPromptCacheKey |

Empty keys fail before client construction. Caller-supplied verified transport is retained.
...
| Format gate | OpenAI, OpenAI Responses; unsupported values fail model validation before I/O |
...
WithPromptCacheKey emits OpenAI prompt_cache_key; the counter forwards the option to count requests.

## Counters, errors, and retries

NewCounter uses exact responses/input_tokens and validates duplicate, missing, negative, and non-integer counts.
...
import (
    ...
    openai "github.com/looprig/llm/providers/openai"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("openai"), model.APIFormatOpenAIResponses, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := openai.New(selected, auth.APIKey(os.Getenv("OPENAI_API_KEY")), openai.WithPromptCacheKey("stable-key"))
...
- [client.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/openai/client.go)
- [counter.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/openai/counter.go)
- [errors.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/openai/errors.go)
- [options.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/openai/options.go)
...
- [client_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/openai/client_test.go)
- [counter_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/openai/counter_test.go)
