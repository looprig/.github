---
id: guides/inference/providers/xai
title: xAI
description: Configure the xAI provider from its released llm package, including endpoint, auth, formats, capabilities, counters, and failure boundaries.
audience: developer
section: guides
order: 54
...
Dual-codec xAI client routing Chat or Responses and translating reasoning_text SSE events into shared Responses reasoning events.

## Contract and endpoint

The public constructor is `func New(selected model.Model, key auth.APIKey, options ...Option) (inference.Client, error)`. It binds provider identity `xai` and resolves the base below when Model.BaseURL is empty, unless the dynamic rule says otherwise.

| Field | Source-backed behavior |
| --- | --- |
| Package | `github.com/looprig/llm/providers/xai` (package `xai`) |
| Provider | `xai` |
| Formats | OpenAI, OpenAI Responses |
| Default base | https://api.x.ai/v1 |
| Authentication | API key |
| Header or signer | Authorization: Bearer |
| Options | WithReasoning; WithServiceTier; WithPromptCacheKey |

Unknown API format is rejected before the request reaches the endpoint.
...
WithPromptCacheKey adds xAI prompt-cache key to the selected request body.
...
import (
    ...
    xai "github.com/looprig/llm/providers/xai"
)
func invoke() error {
    // Resolve credentials from an application-owned lease or environment, never model.Model.
    selected := model.CustomModel(model.ProviderName("xai"), model.APIFormatOpenAIResponses, "", "model-name")
    // Construction validates provider policy and binds its endpoint and codec.
    client, err := xai.New(selected, auth.APIKey(os.Getenv("XAI_API_KEY")), xai.WithPromptCacheKey("stable-key"))
...
- [client.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/xai/client.go)
- [counter.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/xai/counter.go)
- [options.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/xai/options.go)
...
- [client_test.go](https://github.com/looprig/llm/blob/107b378c3882c0a99ad98e36d89e542c5461bc55/providers/xai/client_test.go)
