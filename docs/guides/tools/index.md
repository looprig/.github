...
- the Task tools bundle, exposed as `TaskCreate`, `TaskUpdate`, `TaskGet`, and `TaskList`,
- the supervised process companion tools `ProcessOutput`, `ProcessInput`, and `ProcessStop`, and
- `read_tool_result`, which lets the model page through a large tool result that Harness retained in full.
...
Every standard definition also declares its capture safety, which tells Harness whether a large result streams into durable capture or is fully built in memory first. When a composition wires tool-result retention, the model sees a bounded preview and can read the rest with `read_tool_result`. [Large results and capture](/docs/guides/tools/core-concepts#large-results-and-capture) explains the contract.
