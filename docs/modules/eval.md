...
Eval is useful on its own when an application needs to run cases against a target, score observations, and write redacted reports. Within Looprig, it evaluates [Inference](/docs/modules/inference) targets and can score message threads produced by a [Harness](/docs/modules/harness) application. Eval defines the evaluation domain; Pluto packages that domain into a complete qualification product.
